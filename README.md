...
Interactive mobile app and web page illustrating the ideas from the ML/AI course essays, read alongside the essays themselves.
...
Each lecture comes with two PDFs, shown beside the notes on wide screens: the **essay**, a prose write-up of the lecture, and the **handout**, a condensed outline of the essay with the formulas.
...
- `src/gd3d/sceneHtml.ts` — The Three.js gradient-descent scene
- `src/components/PdfPane.tsx` — pdf.js viewer for the embedded lecture PDFs
- `assets/lectures/` — The lecture PDFs; `sh scripts-embed-pdfs.sh` regenerates `src/lectures/pdfData.ts`
